...
export class DashboardStore extends ComponentStore<DashboardState> {
    ...
    readonly save = (dashboard:Dashboard, nextFn: {(response:any): void} | null = null, errorFn: {(response:any): void} | null = null) => {        
        ...
        const updateFn = dashboard?.dashboardId ? ([response, dashaboards]: [any, Dashboard[]]) => this.patchState({

            dashboards: dashaboards.map(t => response.dashboard.dashboardId == t.dashboardId ? response.dashboard : t)
        })
    ...
    readonly delete = this.effect<Dashboard>(
        exhaustMap((dashboard) => this._dashboardService.delete({ dashboard: dashboard }).pipe( 
            withLatestFrom(this.select(x => x.dashboards )),           
            tapResponse(
                ([_, dashaboards]) => this.patchState({ dashboards: dashaboards.filter(t => t.dashboardId != dashboard.dashboardId )}),
                noop
            )
        ))
    );
